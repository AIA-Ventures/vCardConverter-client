(function (global, factory) {
  ...
  const getInput = function getInput(selector) {
    return typeof selector === "string"
      ? document.querySelector(selector)
      : selector();
  };
  const createResultsList = function createResultsList(renderResults) {
  ...
  const highlight = function highlight(value) {
    return "<span class="
      .concat(select.highlight, ">")
      .concat(value, "</span>");
  };
  const addResultsToList = function addResultsToList(
    resultsList,
    dataSrc,
    resultItem
  ) {
    ...
    dataSrc.forEach(function (event, record) {
      ...
      resultItem.content
        ? resultItem.content(event, result)
        : (result.innerHTML = event.match || event);
      fragment.appendChild(result);
    });
    resultsList.appendChild(fragment);
  };
  const clearResults = function clearResults(resultsList) {
    resultsList.innerHTML = "";
  };
  const onSelection = function onSelection(
    event,
    field,
    resultsList,
    feedback,
    ...
    feedback({
      ...
      selection: resultsValues.list.find(function (value) {
        if (event.keyCode === keys.ENTER) {
          ...
          return (
            value.index ===
            Number(event.currentTarget.getAttribute(dataAttribute))
          );
        }
      }),
    });
  ...
  var autoComplete = function () {
    function autoComplete(config) {
      ...
      var select_base = selector.slice(1)
      select = {
        resultsList: `${select_base}_list`,
        result: "autoComplete_result",
        highlight: `${select_base}_highlighted`,
        selectedResult: "autoComplete_selected",
      };
